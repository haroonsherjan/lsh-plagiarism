...
public class MyTree {
 ...
 private boolean isAncestor(int A, int B)
 {
  ...
  else  isInLeft = isAncestor(myTree[A].getLeft(),B);
  //Traverse the right subtree if it's not empty
  if (myTree[A].getRight()==-1) {
  }
  else isInRight = isAncestor(myTree[A].getRight(),B);
 ...
 {
  //Iterate through every node and determine whether there exists a node with left child B and right child A.
  for (int i = 0; i<size; i++)
  {
   if (myTree[i].getRight()==A && myTree[i].getLeft()==B)
    return true;
  }

  return false;
 }//end method isRightSibling
}//end class MyTree.
import java.util.*;
