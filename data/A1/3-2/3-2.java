...
class MyNode {
 ...
 public void setLeftChild (MyNode node){
  leftChild = node;
 }
 public void setRightChild(MyNode node){
  rightChild = node;
 }
 ...
 */
 ...
 public void updateNode(int node, int left, int right){
  if (left != -1) {
   myTree[node].setLeftChild(myTree[left]);
   myTree[left].setParent(myTree[node]);
  }
  if (right != -1) {
   myTree[node].setRightChild(myTree[right]);
 ...
 public boolean isPathSafe(int start, int end){
  ...
  if (!curNode.isSafe()) return false;
  do {
    if (curNode.isSafe()){
      path.add(curNode);
    }
   ...
   if (curNode.getId() == end && curNode.isSafe()) {
    return true;
   }
 ...
 public void PrintTree() {
  for (int i = 0; i < myTree.length; i++) {
   System.out.print(i + ":");
   if (myTree[i].getLeftChild() == null)
    System.out.print("NULL:"); 
   else 
    System.out.print(myTree[i].getLeftChild().getId() + ":");
   if (myTree[i].getRightChild() == null)
    System.out.println("NULL"); 
   else 
    System.out.println(myTree[i].getRightChild().getId());
  }
 }
}
